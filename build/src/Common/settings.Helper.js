import { FrameworkControlTags, } from "asFramework/src/index";
import { AS_MaterialDesign_SettingKeywords, TargetType, } from "./commonSettings";
...
export function getPageSettings(as) {
    var _a;
    let retValue = {};
    ...
    for (let index = 0; index < cachedPageSettingControls.length; index++) {
        ...
        if (!c.smartobject.exists) {
            ...
            (_a = c.smartobject.items) === null || _a === void 0 ? void 0 : _a.filter((soi) => soi.SubCategory == AS_MaterialDesign_SettingKeywords.pageSettings).forEach((soi) => {
...
function findAndCacheAllPageSettingControls(as) {
    ...
    let pageSettingsControls = as.collections.viewInstanceControls.filter((c) => c.name.includes(FrameworkControlTags.pageSettings));
 ...
 */
export function setupCallbackForWhenTagSettingsChange(as, tagName, settingsChangedCallback) {
...
export function getProcessedTargetsForTagName(as, tagName) {
    ...
    let retValue = {
        controls: processControls(as, tagName, pageControlTypeSettings, TargetType.controls),
        viewsInstances: processControls(as, tagName, pageControlTypeSettings, TargetType.views),
        tag: tagName
    };
...
export function refreshSettings(target) {
    var _a;
...
function processControls(as, tagName, targetingSettings, type) {
    ...
    taggedAndTargetedControlPairs.forEach((targetedControl) => {
        ...
        (_a = targetedControl.appliedTargetSettings) === null || _a === void 0 ? void 0 : _a.forEach((targetToApply) => {
            ...
            applySettingsToObject(newProcessedTargetControl.settings, targetToApply.settings, "settings");
        });
        ...
        if (siblingControlResult) {
            ...
            if (siblingControlResult.settings) {
                ...
                applySettingsToObject(newProcessedTargetControl.settings, siblingControlResult.settings, "settings");
            }
        }
 ...
 */
export function getControlSiblingSettings(taggedControl, mdControl, type) {
    var _a;
    ...
    let controlsToSearch = type == TargetType.controls
    ...
    let settingsControl = controlsToSearch === null || controlsToSearch === void 0 ? void 0 : controlsToSearch.filter((c) => c.name.includes(settingsControlName)).find((c) => c.name.includes(AS_MaterialDesign_SettingKeywords.mdSiblingControlSetting));
    ...
    for (let index = 0; index < processedTarget.length; index++) {
        ...
        if (!templatesNameAsLowercase.includes("!default")) {
            if (templates.default) {
                ...
                applySettingsToObject(combinedTemplateSetting, templates === null || templates === void 0 ? void 0 : templates.default, "settings");
            }
        }
        for (let index = 0; index < templatesNameAsLowercase.length; index++) {
            ...
            if (templateSettings) {
                ...
                applySettingsToObject(combinedTemplateSetting, templateSettings, "settings");
            }
        }
...
//# sourceMappingURL=settings.Helper.js.map