...
import { getNestedProperty } from "../Helpers/ObjectHelper";
import { applyEvalCommands, applyEvalCommandsV2 } from "./expressionsToString";
// import { PopulateFrameworkHelp } from "./EndUserInfo";
...
export class Framework implements IFramework {
  ...
  public static async registerExtensionModule(
    ...
    return Framework.initialize({
      ...
      //log details about the module and the error
      console.groupCollapsed(`%c Error registering module [${moduleName}]`, "color:red", e);
  ...
  async initialize(): Promise<IFramework> {
    ...
    return this.processK2SmartFormsXML(this.window).then(() => {
      ...
      applyEvalCommands(this);
      applyEvalCommandsV2(this);
   ...
   */
  runPostInitializeRules() {
    // PopulateFrameworkHelp();
    ...
    if (viewInstanceName) {
      let retValue =
        this.collections.rules?.rules?.filter(
          (r) =>
            r.ruleFriendlyName == name && r.parentViewInstance?.name == viewInstanceName
        ) || [];
   ...
   */
    getControlsByFieldPropertyConfigurationName(
      ...
      let { name, viewInstance } =
        this.getNameAndViewInstanceFromConfigurationString(
          configurationName,
          referenceViewInstance || undefined
        );


     let x =   genericSearchPropContains(
          this.collections.viewInstanceControls,
          "field.propertyName",
          name,
          viewInstance
...
export function genericSearchPropEqual(
  ...
  return array.filter((c: any) => {
    ...
    if (property) {
      if (viewInstanceName)
        return property == value && c.parent.name == viewInstanceName;
