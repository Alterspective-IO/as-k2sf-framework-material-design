...
export function updateAllK2ControlsBoundToGridColumns(
  ...
  if (pack.processedSettings.k2control_to_bind_rowData) {

    window.as
      .getControlsByConfigurationName(
        pack.processedSettings.k2control_to_bind_rowData,
        pack.viewInstance
      )
      .forEach((c) => {
        c.value = JSON.stringify(dataObject, null, 2);
      });

  }

  if (pack.processedSettings.autoBindToViewControls) {
